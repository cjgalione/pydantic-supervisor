  ...
  cat > "$ENV_PATH" <<EOF
{
  ...
  ,"tempo_max_attribute_bytes": $TEMPO_MAX_ATTRIBUTE_BYTES
  ,"tempo_grpc_max_msg_size": $TEMPO_GRPC_MAX_MSG_SIZE
...
echo "==> Tempo limits: ingest_rate=${TEMPO_INGEST_RATE_LIMIT_BYTES}B/s burst=${TEMPO_INGEST_BURST_SIZE_BYTES}B max_trace_bytes=${TEMPO_MAX_BYTES_PER_TRACE} max_attr_bytes=${TEMPO_MAX_ATTRIBUTE_BYTES} grpc_max_msg=${TEMPO_GRPC_MAX_MSG_SIZE} search_slo=${TEMPO_SEARCH_DURATION_SLO_SECONDS}s throughput_slo=${TEMPO_SEARCH_THROUGHPUT_BYTES_SLO}B"
render_tempo_config
compose_up_wait
